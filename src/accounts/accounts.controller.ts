...
export class AccountsController {
    ...
    @Post('')
    @HttpCode(201)
    public async createAccount(@Req() req : Request, @Body() account : AccountRequest) {

        const userId = req["user"].user_id;
        const created_at = new Date(Date.now());

        const accountToInsert = AccountMapper.toAccountSaveDto(account, userId, created_at)
    ...
    public async listAccounts(@Req() req: Request) {
        ...
        if(user.user_role == UserRole.manager) {
            ...
            return accounts.map((account) => AccountMapper.toAccountResponseDto(account));
        }
    ...
    public async getAccountById(@Req() req: Request) {
    ...
    @HttpCode(204)
    @UseGuards(AccountOwnershipGuard)
    public async deleteAccount(@Param('accountId') accountId : string) {
    ...
    public async pauseAccount(@Param('accountId') accountId : string) {
        this.AccountsService.changeAccountStatus(accountId, AccountStatus.paused);
    }
    ...
    public async cancelAccount(@Param('accountId') accountId : string) {
        this.AccountsService.changeAccountStatus(accountId, AccountStatus.cancelled);
    }
